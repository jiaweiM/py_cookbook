...
'''
通过 append() 添加list, 参数 list 被当做一个元素添加进来，构成嵌套的 list.
要将 list 的元素依次添加进来，用 extend()
'''
...
'''
The index() 查找指定元素，返回其index，如果存在多次，返回第一个位置。
如果对应元素不存在，抛出 ValueError.
在 Tuple 和 String 中有相同的方法。
'''
